...
#define an_matriz_traspuesta_matriz(A,B,C,NfilA,NcolA,NcolB) {int ml,mk,mu;\
          for(mu=0;mu<NfilA;mu++) \
            for(mk=0;mk<NcolB;mk++){\
...
Array2D< real > an_inversa(const Array2D< real > &A ) ;

// FUNCION PARA RESOLVER SISTEMAS POR EL METODO DE GAUSS
Array1D< real > an_gauss(const Array2D< real > &A, const Array1D< real > &b);
